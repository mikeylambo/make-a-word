...
need(main, 'if (round.mode === "daily") save.daily[todayKey()] = Math.max', 'Daily best score persistence is missing or can regress');
...
need(main, 'remaining * 60', 'Burn leftover-letter cost is missing');
...
console.log(`Progression gate passed: ${versions.length} Daily schedule version(s), ${trials.length} ordered Trials, Burn board transitions, and local multiplayer completion invariants are intact.`);
