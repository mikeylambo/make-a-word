...
for (const phrase of phrases) {
  ...
  if (phrase.journeyOrder !== undefined) {
    ...
    if (!Array.isArray(phrase.medals) || phrase.medals.length !== 3 || !(phrase.medals[0] < phrase.medals[1] && phrase.medals[1] < phrase.medals[2])) {
      errors.push(phrase.id + ': Journey medals must contain three ascending scores');
    }
  }
}
...
journey.forEach((phrase, index) => {
  if (phrase.journeyOrder !== index + 1) errors.push(phrase.id + ': Journey order must be contiguous (expected ' + (index + 1) + ')');
});

const burnCount = phrases.filter((phrase) => phrase.burnSolution?.length).length;
if (burnCount < 40) errors.push('Burn needs at least 40 verified phrases; found ' + burnCount);
if (journey.length < 20) errors.push('Journey needs at least 20 stages; found ' + journey.length);
...
if (errors.length) {
  ...
  console.log('Phrase Doctor passed: ' + phrases.length + ' phrases · ' + burnCount + ' verified Burn boards · ' + journey.length + ' Journey stages');
